...
subtitle: 'Tutorial 3: model and forecase evaluation'
...
post_contrasts <- avg_comparisons(model2, 
                        newdata = datagrid(log_spawners = log(c(3000, 5000)))) %>%
  posteriordraws() 
...
These summaries are informative and not too surprising. The AR1 / Poisson model tends to give better approximate forecasts (lower Discrete Rank Probability Scores) at all horizons. The `compare_mvgams` function can be used to automate this process by rolling along a set of timepoints for each model. But we have to be careful interpreting these comparisons, as the AR1 model is extending out from a well-estimated 'state' at each evaluation timepoint (the GP trend is smoother and so the starting 'state' for each rolling evaluation is not as unrealistically perfect). Overall we can't put too much faith in these comparisons, unfortunately.  
