...
link & = \mu = \alpha + \boldsymbol{X}\beta + z \\
expected & = \mathbb{E}(\boldsymbol{Y}|\mu, \sigma) = \mu
\end{align*}
...
link & = log(\lambda) = \alpha + \boldsymbol{X}\beta + z \\
expected & = \mathbb{E}(\boldsymbol{Y}|\lambda) = exp(\lambda)
\end{align*}
---
...
\begin{align*}
\boldsymbol{Y} & \sim \text{LogNormal}(\mu, \sigma) \\
link & = \mu = \alpha + \boldsymbol{X}\beta + z  \\
expected & = \mathbb{E}(\boldsymbol{Y}|\mu, \sigma) = exp(\mu + \frac{\sigma^2}{2})
\end{align*}
...
class: middle center
### We won't dwell much on point-based measures because ecological predictions and their associated management decisions are inherently ([but see this video for more details](https://www.youtube.com/watch?v=ek5xLEoQN3E&t=5s)) *uncertain*
<br>
...
$$MSIS = (U_{t} - L_{t}) + \frac{2}{\alpha}(L_{t} - y_{t})\mathcal{1}(y_{t} < L_{t}) + \frac{2}{\alpha}(y_{t} - U_{t})\mathcal{1}(y_{t} > U_{t})$$
Where:
...
title(main = bquote(MSIS[80]~'= '~.(msis)))
...
User chooses among the Scaled Interval Score (`sis`), log score (`elpd`), CRPS (`crps`), DRPS (`drps`) and a multivariate score (`variogram`; more on this in the next lecture)
