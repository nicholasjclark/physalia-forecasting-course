...
date: "0900&ndash;1200 CET Wednesday 26th March, 2024"
output:
...
knitr::opts_chunk$set(cache = FALSE, dev = 'png', echo = TRUE,
                      dpi = 550,
...
library('tidybayes')
library('gratia')
library('ggplot2'); theme_set(theme_classic(base_size = 18, base_family = 'serif'))
...
Two options
- Pass `newdata` into the `mvgam()` function for automatic probabilistic forecasts through `Stan`
- Produce forecasts outside of `Stan` by passing `newdata` and the fitted model to the `forecast()` function
...
Forecasts computed automatically using the [`generated quantities` block in `Stan`](https://mc-stan.org/docs/reference-manual/program-block-generated-quantities.html)
...
`plot(hindcast(model2))`
```{r echo = FALSE}
plot(hindcast(model2)) +
  theme_classic(base_size = 18, base_family = 'serif')
```
No forecasts available. Now what?
---
...
```{r trend_draws, fig.show='hide', fig.height=6.2, fig.width=1.9*6.2, echo=FALSE, dev='svg'}

# view posterior draws of the trend
plot(model2, type = 'trend', realisations = TRUE, 
     n_realisations = 10) +
  theme_classic(base_size = 17, base_family = 'serif')
```
...
```{r trend_extrap, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2,echo=FALSE, dev = 'svg'}
...
forecast_draw = sim_gp(trend_draw = trend_draws[2,],
                       alpha = alpha_draws[2,],
                       rho = rho_draws[2,],
                       h = 30)
plot(1, type = 'n', bty = 'l',
     xlim = c(1, 120), 
     ylim = range(c(trend_draws[2,]),
                  forecast_draw),
     ylab = 'One trend draw', xlab = 'Time')
lines(trend_draws[2,], col = 'gray70', lwd = 3.5)
...
---
`plot(model2, type = 'trend', n_realisations = 4)`
```{r echo = FALSE}
set.seed(20)
plot(model, type = 'trend', newdata = poisdat$data_test,
...
`plot(model2, type = 'trend', n_realisations = 8)`
```{r echo = FALSE}
set.seed(20)
plot(model, type = 'trend', newdata = poisdat$data_test,
...
`plot(model2, type = 'trend', n_realisations = 30)`
```{r echo = FALSE}
set.seed(20)
plot(model, type = 'trend', newdata = poisdat$data_test,
...
`plot(model2, type = 'trend', n_realisations = 60)`
```{r echo = FALSE}
set.seed(20)
plot(model, type = 'trend', newdata = poisdat$data_test,
...
`plot(forecast(model2, type = 'link'), realisations = TRUE)`
```{r echo = FALSE}
library(mvgam)
...
class: middle center
### Forecasting is easier if `newdata` passed to `mvgam()`, but this results in a larger model object and requires test data be available now
<br>
### When testing data not available, you can generate forecasts for new data later using `forecast()` (note, `time` values in `newdata` must follow immediately from `time` values in original training data)
<br>
...
---
class: middle center
### How should *you* use these predictions?
<br>
### 1. Don't interpret coefficients. Rather, use predictions on both the link and expectation scale to understand the model better
<br>
### 2. Compare predictions on the response scale to the observed data using posterior predictive checks and `loo()` to diagnose problems
<br>
### 3. Make out-of-sample predictions and evaluate with proper scoring rules
---
...
```{r linkpreds, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2, echo=FALSE, results='hide', dev='svg'}
...
This is the most confusing type of prediction, as using `predict(model, type = 'response')` for models fitted in `gam()` or `glm()` actually return .emphasize[*expectations*]
...
```{r exppreds, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2,echo=FALSE, results='hide', dev='svg'}
...
These are the most often used for evaluating forecasts
...
```{r resppreds, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2, echo=FALSE, results='hide', dev='svg'}
...
```{r ppc1, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2,echo=FALSE, results='hide', dev='svg'}
...
```{r ppc2, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2,echo=FALSE, results='hide', dev='svg'}
...
```{r ppc3, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2,echo=FALSE, results='hide', dev='svg'}
...
A RW with no seasonality will fit the data *very well*, but will not *generalize* well
---
...
---
`plot_predictions(model_good, by = 'time')`
```{r echo=FALSE, message=FALSE}
plot_predictions(model_good, by = 'time', points = 0.6) +
  theme_classic(base_size = 18, base_family = 'serif')
```

---

`plot_predictions(model_bad, by = 'time')`
```{r echo=FALSE, message=FALSE}
plot_predictions(model_bad, by = 'time', points = 0.6) +
  theme_classic(base_size = 18, base_family = 'serif')
```
...
A much lower Expected Log Predictive Density (ELPD) for `model_bad` suggests that this model would not do as well when tasked with predicting against a *new* set of observations
---
class: middle center
...
- Nonlinear terms, nonlinear link functions, interaction effects and observation parameters all make these effects nearly impossible to access from coefficients alone
- Most software emphasizes coefficients and *p*-values over meaningful interpretations

---
## `predict.mvgam()`
Pass `newdata` consisting of particular covariate values that represent scenarios you'd like to explore
...
```{r condsmooth, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2,results='hide', dev='svg', echo=FALSE}
...
```{r derivsmooth, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2,results='hide', dev='svg', echo=FALSE}
# use plot_slopes() to visualise conditional effects 
# on the scale of the response
plot_slopes(model, variables = 'season',
            by = 'season', type = 'expected',
            process_error = FALSE) +
...
```{r eval=FALSE}
# use plot_slopes() to visualise rates of change 
# on the expectation scale of the response
plot_slopes(model, variables = 'season',
            by = 'season', type = 'expected',
            process_error = FALSE) +
  geom_hline(yintercept = 0, linetype = 'dashed')
...
.panel[.panel-name[Plot]
.center[![](lecture_4_slidedeck_files/figure-html/derivsmooth-1.svg)]
...
## Finding peak growth
```{r}
# estimate when the `season` smooth grows at its fastest rate
max_growth = function(hi, lo, x) {
  dydx <- (hi - lo) / 1e-6; x[dydx == max(dydx)][1]
}

comparisons(
  model, type = 'link',
  newdata = datagrid(season = seq(0, 12, length.out = 200)),
  variables = list("season" = 1e-6), comparison = max_growth
)
```
...
```{r contrast, fig.show='hide',fig.height=6.2, fig.width=1.9*6.2,results='hide', dev='svg', echo=FALSE}
...
---
class: middle center
### The ability to readily interpret models from `mvgam` and `brms` 📦's is a .emphasize[*huge advantage*] over traditional time series models. See [my blogpost on interpeting GAMs for more examples](https://ecogambler.netlify.app/blog/interpreting-gams/)
<br>
...
## How does `model_good` perform?
.panelset[
.panel[.panel-name[Code]

```{r fc_good, fig.show='hide', fig.height=6.2, fig.width=1.9*6.2, dev = 'svg', echo=FALSE}
fc_good <- forecast(
  model_good,
...
plot(fc_good) +
  theme_classic(base_size = 17, base_family = 'serif') +
  labs(subtitle = paste0('DRPS = ',
                      round(sum(score(fc_good)$all_series$score),
                            4)))
```
...
.panel[.panel-name[Plot]
.center[![](lecture_4_slidedeck_files/figure-html/fc_good-1.svg)]
...
## How does `model_bad` perform?
.panelset[
.panel[.panel-name[Code]

```{r fc_bad, fig.show='hide', fig.height=6.2, fig.width=1.9*6.2, dev = 'svg', echo=FALSE}
fc_bad <- forecast(
  model_bad,
  newdata = poisdat$data_test
)
plot(fc_bad) +
  theme_classic(base_size = 17, base_family = 'serif') +
  labs(subtitle = paste0('DRPS = ',
                      round(sum(score(fc_bad)$all_series$score),
                            4)))
```
...
.panel[.panel-name[Plot]
.center[![](lecture_4_slidedeck_files/figure-html/fc_bad-1.svg)]
...
## How would an *ensemble* perform?
.panelset[
.panel[.panel-name[Code]

```{r fc_ens, fig.show='hide', fig.height=6.2, fig.width=1.9*6.2, dev = 'svg', echo=FALSE}
fc_good <- forecast(
  model_good,
  newdata = poisdat$data_test
)
...
ens <- ensemble(fc_good, fc_bad)
plot(ens) +
  theme_classic(base_size = 17, base_family = 'serif') +
  labs(subtitle = paste0('DRPS = ',
                      round(sum(score(ens)$all_series$score),
                            4)))
```
...
.panel[.panel-name[Plot]
.center[![](lecture_4_slidedeck_files/figure-html/fc_ens-1.svg)]
