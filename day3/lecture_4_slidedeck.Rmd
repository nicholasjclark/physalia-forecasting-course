...
`predict()` does not know the estimates of the latent states; rather it tells what we might expect if we see these same covariate values .emphasize[*sometime in the future*]

`mus` includes actual state estimates .emphasize[*at each point in the training data*] (hindcasts), so it is has less uncertainty
