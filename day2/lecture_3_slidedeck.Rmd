...
date: "0900&ndash;1200 CET Tuesday 25th March, 2025"
output:
...
library('mvgam')
library('gratia')
library('ggplot2'); theme_set(theme_bw(base_size = 18, base_family = 'serif'))
...
plot_mvgam_series(data = gausdat$data_train, 
...
---
background-image: url('../day1/lecture_2_slidedeck_files/figure-html/basis-functions-weights-1.svg')
...
patchwork::wrap_plots(
  plot(forecast(autogam)) +
    labs(y = 'Predictions',
         x = '',
         title = 'k = 6'),
  plot(forecast(autogam2)) +
    labs(y = '',
         x = 'time',
         title = 'k = 15'),
  plot(forecast(autogam3)) +
    labs(y = '',
         x = '',
         title = 'k = 50'),
  nrow = 1
...
plot(forecast(mod_example)) +
  labs(y = 'Predictions', x = 'Time') +
  theme_classic(base_size = 17, base_family = 'serif')
```
...
portal_data %>%
  ...
  dplyr::mutate(`relative abundance` = pmin(0.99, captures / total),
                relabund = pmin(0.99, captures / total)) %>%
  dplyr::ungroup() %>%
  dplyr::filter(series == 'DM') %>%
  droplevels() -> dm_data
plot_mvgam_series(data = dm_data, y = 'relative abundance') &
  theme_bw(base_size = 15, base_family = 'serif')
```
...
```{r echo=FALSE}
mypar()
plot(mod_beta, type = 'smooth', trend_effects = TRUE)
```
...
```{r beta_ndvi, fig.show='hide', fig.height=6.2, fig.width=1.9*6.2,  message=FALSE,warning=FALSE, echo= FALSE, dev = 'svg'}
...
```{r beta_mintemp, fig.show='hide', fig.height=6.2, fig.width=1.9*6.2,  message=FALSE,warning=FALSE, echo=FALSE, dev = 'svg'}
...
```{r beta_both, fig.show='hide', fig.height=6.2, fig.width=1.9*6.2,  message=FALSE,warning=FALSE, echo=FALSE, dev = 'svg' }
...
```{r rw_sim, echo=FALSE,fig.show='hide', fig.height=6.2, fig.width=1.9*6.2,  echo=FALSE, dev = 'svg'}
# set seed for reproducibility
set.seed(1111)
...
```{r ar_sim, echo=FALSE,fig.show='hide', fig.height=6.2, fig.width=1.9*6.2, echo=FALSE, dev = 'svg'}
# set seed for reproducibility
set.seed(1111)
...
```{r ndvi_time, echo=FALSE,fig.show='hide', fig.height=6.2, fig.width=1.9*6.2,  warning=FALSE, message=FALSE, dev = 'svg'}
...
```{r ndvi_time_brm,fig.show='hide', fig.height=6.2, fig.width=1.9*6.2, warning=FALSE, message=FALSE, dev = 'svg'}

# use brms' conditional_effects to view predictions
plot(conditional_effects(brm_beta_dyn, effects = c('time:mintemp')),
                    theme = theme_classic(base_size = 17, base_family = 'serif'),
                    mean = FALSE,
                    rug = TRUE)
```
