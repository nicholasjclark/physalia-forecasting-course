...
date: "0900&ndash;1200 CET Tuesday 5th September, 2023"
output:
...
```{r ar_sim2, echo=FALSE,fig.show='hide', fig.height=4.25, fig.width=1.9*4.25,echo=FALSE}
set.seed(1111)
T <- 100
...
---
## Enforcing AR stationarity
Enforcing stationarity (by restricting $|\phi| < 0$) is the default behaviour in [`brms` 📦](https://paul-buerkner.github.io/brms/reference/ar.html)
...
## Enforcing AR stationarity
This .emphasize[is not] the default behaviour in `mvgam` 📦. 
...
```{r echo = FALSE}
library(mvgam)
load('cache_not_for_upload/t_data.rds')
multipar()
plot_mvgam_series(data = t_data$data_train,
                  newdata = t_data$data_test)
```
...
---
## RW MCMC diagnostics `r fontawesome::fa("face-smile")`

.small[
```{r echo = FALSE}
mvgam:::check_all_diagnostics(t_mod_rw$model_output)
```
...
## AR1 MCMC diagnostics `r fontawesome::fa("face-sad-tear")`

.small[
```{r echo = FALSE}
mvgam:::check_all_diagnostics(t_mod_ar1$model_output)
```
...
mypar()
plot_mvgam_smooth(t_mod_rw)
title(main = 'RW', line = 0.5)
plot_mvgam_smooth(t_mod_ar1)
title(main = 'AR1', line = 0.5)
```
...
```{r echo = FALSE}
mypar()
ar <- mvgam:::mcmc_chains(t_mod_ar1$model_output, 'ar1')
myhist(ar, xlab = expression(phi[1]),
       main = 'AR1')
abline(v = 1, lwd = 3.5, col = 'white')
abline(v = 1, lwd = 2.75)
```
...
layout(matrix(1:2, nrow = 2))
mypar()
plot(t_mod_rw, type = 'trend', newdata = t_data$data_test,
     ylab = '', xlab = '')
title(main = 'RW', line = 0.5)
plot(t_mod_ar1, type = 'trend', newdata = t_data$data_test,
     ylab = '')
title(main = 'AR1', line = 0.5)
mtext('Trend', side = 2, outer = TRUE, line = -2.5, las = 0, cex = 1.5)
```
...
layout(matrix(1:2, nrow = 2))
mypar()
plot(t_mod_rw, type = 'forecast', newdata = t_data$data_test,
     ylab = '', xlab = '')
title(main = 'RW', line = 0.5)
plot(t_mod_ar1, type = 'forecast', newdata = t_data$data_test,
     ylab = '')
title(main = 'AR1', line = 0.5)
mtext('Predictions', side = 2, outer = TRUE, line = -2.5, las = 0, cex = 1.5)
```
...
layout(matrix(1:2, nrow = 2))
mypar()
rwsig <- mvgam:::mcmc_chains(t_mod_rw$model_output, 'sigma')
arsig <- mvgam:::mcmc_chains(t_mod_ar1$model_output, 'sigma')
myhist(rwsig, xlim = c(0, 1), xlab = '',
       main = 'RW')
myhist(arsig, xlim = c(0, 1), xlab = expression(sigma[error]),
       main = 'AR1')
```
---
class: middle center
...
## Estimation in `brms`
In `brms`📦, use the [`gp` function](https://paul-buerkner.github.io/brms/reference/gp.html) with `time` as the covariate
...
Requires user to set the length scale $\rho$, as the function is approximated using a low-rank GP smooth from the `mgcv`📦
