...
download_file(
  ...
  output_name = "Tutorial2_exercises",
...
```{r Inspect missing values in mintemp_lag4}
head(model_data, 6)
```
...
```{r Plot forecasts of the temporal smooth, warning=FALSE, message=FALSE}
...
model0b <- xfun::cache_rds(mvgam(count ~ s(time, bs = 'bs', k = 8) + 
                  mintemp_lag4 + 
                  ndvi_ma12,
                family = poisson(),
                data = data_train,
...
```{r Plot extrapolated temporal functions using newdata, warning=FALSE}
plot_predictions(
  model0, 
  by = 'time',
...
```{r Residual quantile-quantile plot, warning=FALSE}
...
log(\lambda_t) & = f(\boldsymbol{mintemp})_t + \beta_{ndvi} * \boldsymbol{ndvi}_t + z_t \\
z_t & \sim \text{Normal}(ar1 * z_{t-1}, \sigma_{error}) \\
ar1 & \sim \text{Normal}(0, 1)[-1, 1] \\
\sigma_{error} & \sim \text{Exponential}(2) \\
f(\boldsymbol{mintemp}) & = \sum_{k=1}^{K}b * \beta_{smooth} \\
...
Of course there may be situations where we really do have unequal temporal spacing among observations and we don't want to force a model into equal spacing by filling missing timepoints with `NA`s. In this case, `mvgam` can be used with a Continuous Time Autoregressive process:

\begin{align*}
z_{i,t} & \sim \text{Normal}(ar1_i^{distance} * z_{i,t-1}, \sigma_i) \end{align*}

Where $distance$ is a vector of non-negative measurements of the time differences between successive observations. See the [**Examples** section in `?CAR`](https://nicholasjclark.github.io/mvgam/reference/RW.html){target="_blank"} for an illustration of how to set these models up.
...
This is *much* better extrapolation behaviour compared to splines. Using `how_to_cite()` for this model will give you information on the approximate GPs and how they are estimated:
```{r}
...
<iframe style="aspect-ratio: 16 / 9; width: 100% !important;" src="https://www.youtube.com/embed/Y2ZLt4iOrXU?start=1" data-external = "1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
</center>
