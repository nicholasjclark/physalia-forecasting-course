...
Why is the `brms` model struggling here? Most likely this is happening because any rows in `data_train` that have `NA` in the outcome variable (`count`) are being thrown out before model fitting. There are quite a few of these:
```{r}
length(which(is.na(data_train$count)))
```

This means that the `brms` model *thinks* that certain observations are adjacent to one another in `time` when actually they are not. For example, the `brms` model believes that timepoints `5` and `7` are right next to each other, and also that timepoints `12` and `14` are right next to eachother:
```{r}
data_train %>%
