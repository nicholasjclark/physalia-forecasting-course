...
Press the "o" key on your keyboard if you wish to navigate among slides
...
$$\boldsymbol{Y}\sim \text{Normal}(\mu,\sigma)$$
Properties
- Real-valued continuous observations (including any decimal)
- Unbounded (supports $-\infty$ to $\infty$)
- Symmetric spread, controlled by $\sigma$, about the mean $(\mu)$
...
$$\boldsymbol{Y}\sim \text{Normal}(0,2)$$
```{r echo = FALSE}
...
$$\boldsymbol{Y}\sim \text{Normal}(50,20)$$
```{r echo = FALSE}
...
<br/>
<br/>
$$\boldsymbol{Y}\sim \text{Normal}(\alpha + \beta * \boldsymbol{X},\sigma)$$
<br/>
Where:
- $\boldsymbol{X}$ represents a design matrix of .emphasize[covariates] that contribute linearly to variation in $\mu$
- $\alpha$ is an intercept coefficient
- $\beta$ is a vector of regression coefficients
- $\sigma$ controls the spread of the errors about $\mu$
...
class: middle center
### But most real-world ecological observations, including time series, .emphasize[are not Gaussian]
...
$$\boldsymbol{Y}\sim \text{Poisson}(\lambda)$$
Properties
...
## Poisson
$$\boldsymbol{Y}\sim \text{Poisson}(3)$$
```{r echo=FALSE}
...
## Poisson
$$\boldsymbol{Y}\sim \text{Poisson}(50)$$
```{r echo=FALSE}
...
This requires a .emphasize[link function] that transforms from the unbounded scale of the linear predictor to a scale that is appropriate for the parameters being modelled

---

# Modelling the mean
Most GLMs are used to model the conditional mean $(\mu)$
<br/>
<br/>
$$\mathbb{E}(\boldsymbol{Y}|\boldsymbol{X})=\mu=g^{-1}(\alpha+\boldsymbol{X}\beta)$$
<br/>
Where: 
- $\mathbb{E}$ is the *expected value* of $\boldsymbol{Y}$ conditional on $\boldsymbol{X}$
...
<br/>
\begin{align*}
\boldsymbol{Y}_i & \sim \text{Poisson}(\lambda_i) \\
log(\lambda_i) & = \boldsymbol{X}_i \beta \\
& = \alpha + \beta_1 \boldsymbol{x}_{1i} + \beta_2 \boldsymbol{x}_{2i} + \cdots + \beta_j \boldsymbol{x}_{ij}
\end{align*}

Where: 
- $\boldsymbol{X}_{i}$ is the matrix of predictor values at observation $i$
- $\alpha$ is an intercept coefficient
- $\beta$ is a vector of regression coefficients
- $\mathbb{E}(\boldsymbol{Y}_{i}|\boldsymbol{X}_{i})=exp(\alpha+\boldsymbol{X}_{i}\beta)$
...
\boldsymbol{Y}_i & \sim \text{Poisson}(\lambda_i) \\
log(\lambda_i) & = \boldsymbol{X}_i \beta \\
& = \color{darkred}{\alpha + \beta_1 \boldsymbol{x}_{1i} + \beta_2 \boldsymbol{x}_{2i} + \cdots + \beta_j \boldsymbol{x}_{ij}}
\end{align*}
...
$$\boldsymbol{Y}\sim \text{Beta}(\mu,\phi)$$
Properties
- Real-valued continuous observations (including any decimal)
- Both lower and upper bounds (supports $0$ to $1$, noninclusive)
- mean = $\mu$
- precision = $\phi$ (higher $\phi$ = a tighter spread about $\mu$)
...
## Beta
$$\boldsymbol{Y}\sim \text{Beta}(0.5,1)$$
```{r echo=FALSE}
...
## Beta
$$\boldsymbol{Y}\sim \text{Beta}(0.5,15)$$
```{r echo=FALSE}
beta_shapes = function(mu, phi) {
  return(list(shape1 = mu * phi,
              shape2 = (1 - mu) * phi))
...
## Beta
$$\boldsymbol{Y}\sim \text{Beta}(0.75,4)$$
```{r echo=FALSE}
beta_shapes = function(mu, phi) {
  return(list(shape1 = mu * phi,
              shape2 = (1 - mu) * phi))
...
<br/>
\begin{align*}
\boldsymbol{Y}_i & \sim \text{Beta}(\mu_i,\phi) \\
logit(\mu_i) & = \boldsymbol{X}_i \beta \\
& = \alpha + \beta_1 \boldsymbol{x}_{1i} + \beta_2 \boldsymbol{x}_{2i} + \cdots + \beta_j \boldsymbol{x}_{ij}
\end{align*}
...
- $\alpha$ is an intercept coefficient
- $\beta$ is a vector of regression coefficients
- $\mathbb{E}(\boldsymbol{Y}_{i}|\boldsymbol{X}_{i})=logit^{-1}(\alpha+\boldsymbol{X}_{i}\beta)$
...
class: middle center
### GAMs are just fancy GLMs, where some (or all) of the predictor effects are estimated as smooth functions.
<br>
...
<img align="center" width="306" height="464" src="resources/gam_book.jpg" style="float: right; 
  margin: 10px;">
$$\mathbb{E}(\boldsymbol{Y}|\boldsymbol{X})=g^{-1}(\alpha + \sum_{j=1}^{J}f(x_j))$$
...
## Random effects are .emphasize[hierarchical]
<br>
<br>
...
class: middle center
### Hierarchical models .emphasize[learn from all groups at once] to inform group-level estimates
<br>
### This induces .emphasize[regularization], where noisy estimates are pulled towards the overall mean
<br>
...
\begin{align*}
\boldsymbol{Y}_i & \sim \text{Poisson}(\lambda_i) \\
log(\lambda_i) & = \beta_{year[year_t]} * \boldsymbol{year}_{t} \\
...
set.seed(100)
data <- sim_mvgam(n_series = 4, T = 24, prop_missing = 0.3)
data$data_train$x0 <- rnorm(24)
data$data_train$site <- ifelse(
...
head(data$data_train, 8) %>%
  kbl() %>%
  kable_minimal(full_width = FALSE) %>%
  column_spec(4, bold = T, background = '#51247A20') %>%
  column_spec(5, bold = T, background = '#51247A20')
```
...
## `mvgam` with yearly random intercepts
...
# plot as histograms
hist(pop_params$`mu_raw[1]`, main = expression(mu[year]))

hist(pop_params$`sigma_raw[1]`, main = expression(sigma[year]))
```
...
pop_params <- as.data.frame(year_random, 
                            variable = c('mu_raw[1]',
                                         'sigma_raw[1]'))
myhist(pop_params$`mu_raw[1]`, main = expression(mu[year]))
```
...
pop_params <- as.data.frame(year_random, 
                            variable = c('mu_raw[1]',
                                         'sigma_raw[1]'))
myhist(pop_params$`sigma_raw[1]`, main = expression(sigma[year]))
```
]
...
```{r year_smooth_derivs, fig.show='hide', fig.height=4.25, fig.width=1.9*4.25,echo=FALSE}
