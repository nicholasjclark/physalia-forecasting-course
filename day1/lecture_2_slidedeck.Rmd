...
date: "0900&ndash;1200 CET Monday 27th May, 2024"
output:
...
- [Forecasting with Dynamic Generalized Additive Models](https://www.youtube.com/watch?v=0zZopLlomsQ)
...
ggplot(basis(year_smooth$mgcv_model), 
  ...
  geom_borderline(data = smooth_estimates(year_smooth$mgcv_model),
                  inherit.aes = FALSE,
                  mapping = aes(x = year, y = .estimate), linewidth = 2) +
  scale_color_viridis(discrete = TRUE) +
...
```{r echo=FALSE}
multipar()
plot(temp_time_smooth, type = 'smooth')
```
...
```{r echo=FALSE}
mypar()
plot(temp_time_smooth, type = 'forecast')
```
